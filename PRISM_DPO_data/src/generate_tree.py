...
def try_vllm(actor, tokenizer, node, config, terminators, image_path=None, retry_num=0):
    try:
        messages = []
        ...
        user_message = {"role": "user", "content": node.prompt}
        ...
        # Add the trajectory as a single assistant response (concatenated previous model outputs)
        
        prefix_str = ""
        
        if node.trajectory:
            
            trajectory_content = "\n\n".join(node.trajectory)
            
            if trajectory_content.endswith("<|/PROBLEM|>"):
                trajectory_content += "\n\n<|CAPTION|>\n"
                prefix_str = "<|CAPTION|>\n"
            
            elif trajectory_content.endswith("<|/CAPTION|>"):
                trajectory_content += "\n\n<|REASONING|>\n"
                prefix_str = "<|REASONING|>\n"
                
            elif trajectory_content.endswith("<|/REASONING|>"):
                trajectory_content += "\n\n<|OUTPUT|>\n"
                prefix_str = "<|OUTPUT|>\n"
                
            messages.append({"role": "assistant", "content": trajectory_content})
        
        # print("VLLM PROMPT:", messages)
        
        # Convert stop token IDs to strings for VLLM
        stop_strings = []
        for token_id in terminators:
            if token_id != tokenizer.eos_token_id:  # Skip EOS token
                try:
                    stop_str = tokenizer.decode([token_id], skip_special_tokens=False)
                    if stop_str and stop_str not in stop_strings:
        ...
        if node.trajectory:
            request_body = {
                ...
                "top_p": config.top_p,
                "stop": stop_strings,  # Use stop strings instead of token IDs
                "extra_body": {
                    "top_k": config.top_k,
        ...
        completion = actor.chat.completions.create(**request_body)
        # print("VLLM RESPONSE: " + str(completion.choices[0].message.content))
        return prefix_str + completion.choices[0].message.content
        ...
        return try_vllm(actor, tokenizer, node, config, terminators, image_path, retry_num)
...
def generate_action(actor, tokenizer, node:mcts_node, config, image_path=None):
    # logging.info("Ter Tokens:", [tokenizer.convert_tokens_to_ids(stop_token) for stop_token in config.stop_tokens])
    terminators = [
    ...
    if config.generate_mode == "local":
        ...
        # If you have image processing for local models, add it here
        ...
        return try_vllm(actor, tokenizer, node, config, terminators, image_path), 1.0

def _get_reward(prompt, answer, depth, ground_truth, question_type, config, retry_num=0):
    
    try:
        mode = config.mode
        safe_score, helpful_score = evaluate(mode, prompt, answer, depth, ground_truth, question_type)
        ...
        return _get_reward(prompt, answer, depth, ground_truth, question_type, config, retry_num)

def get_reward(node:mcts_node, ground_truth, question_type, config):

    if node.depth == 0:
        return 0  # No reward for root node
    
    answer = node.trajectory[-1]
    depth = node.depth
    
    return _get_reward(node.prompt, answer, depth, ground_truth, question_type, config)
...
def thread_function(prompts_data, config, worker_order):
    ...
    for prompt_index, prompt in enumerate(prompts_data):
        ...
        if config.use_cache and prompt["question"] in cache:
            ...
            mct_data["question"] = prompt["question"]
            mct_data["image"] = prompt["image"]
            
            output_content.append(mct_data)
            continue
        
        question = prompt["question"]
        ground_truth = prompt["response"] if "response" in prompt.keys() else None
        question_type = prompt["type"]
        image_path = prompt["image"] if "image" in prompt.keys() else None
        ...
        for iteration in tqdm(range(config.iterations), desc=f"Worker {worker_order} Prompt {prompt_index} Iter", ncols=100):
            ...
            if selected_node.visited_times != 0 and not selected_node.is_terminal and selected_node.depth < selected_node.max_depth:
                ...
                for _ in range(config.generate_samples_number):
                    # Pass image_path to generate_action
                    action, p = generate_action(actor_model, actor_tokenizer, selected_node, config, image_path)
            ...
            # Evaluate the current node directly instead of doing rollout
            if selected_node.visited_times != 0:
                ...
                # Always evaluate the current node state directly
                feedback = get_reward(selected_node, ground_truth, question_type, config)
                feedback_type = "direct_evaluation"
            
            if feedback == None:
                print("WARNING: Feedback is None for node:", selected_node.prompt)
                feedback = 0  # Default feedback if None
            ...
            while selected_node != None:
                ...
                for end_token in config.end_tokens:
                    if selected_node.trajectory != [] and end_token in selected_node.trajectory[-1]:
                        is_terminal = True
                        break
                # Evaluate current node state directly
                feedback = get_reward(selected_node, ground_truth, question_type, config)
                feedback_type = "direct_evaluation"
...
def main():
    ...
    logging.info("PROMPT DATA LENGTH: " + str(len(prompts_data)))   
    threads = []
